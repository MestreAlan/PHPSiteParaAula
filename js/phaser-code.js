...
function createGame(){	
	...
	while(analisador==0){
		...
		if(desvio==0||perfil[1]==0){
			...
			registro[4]=[player.x,player.y];
			registro[5]=listaPath.slice();
		}else{
			player.x=registro[4][0];
			player.y=registro[4][1];
			listaPath=registro[5];
			analisador=1;
		}
	}
	...
	for(var i=0;i<3;i++){
		var retornoLista;
		if(desvio==0||perfil[1]==0){
			retornoLista = createPathSecond();
		}else{
			listaCaminho=registro[6][i];
			retornoLista=1;
		}
		if(retornoLista==1){
			listaPathTemp = [];
			for(var j=0;j<listaCaminho.length;j++){
				listaPathTemp.push(listaCaminho[j][0]);
			}
			if(desvio==0||perfil[1]==0){
				var verificarQuestionCont = verificadorQuestions(listaPathTemp);
				if(verificarQuestionCont==1&&listaCaminho[listaCaminho.length-1][1]!=portal.x&&listaCaminho[listaCaminho.length-1][2]!=portal.y){
					resposta.push([listaPathTemp,0]);
					registro[6].push(listaCaminho);
				}else{
					i--;
				}
			}else{
...
function verificadorQuestions(listaPathTemp){
	...
	for(var i =0;i<resposta.length;i++){
		if(resposta[i][0]==listaPathTemp.slice()){
			bool=false;
			i=resposta.length;
			j=resposta[i].length;
		}
...
function createQuestion(){
	...
	if(perfil[1]==1&&desvio==1){
		var contadorCriacaoBotao = 0;
		var registro8 = registro[8].slice();
		while(controladorQuestaoDistribuicao.length>0){
			contadorCriacaoBotao++;
			var contadorTemp = registro8.splice(0,1);
			...
			$("#heading"+contadorCriacaoBotao).append(	
				"<h5 class='mb-0'>"+
					"<button class='btn btn-link' data-toggle='collapse' data-target='#collapse"+contadorCriacaoBotao+"' aria-expanded='true' aria-controls='collapseOne' onclick='posiRef="+temp2+";respostaTexto="+temp3+"; desvio=0; funcaoLog(\"Selecionou a alternativa: "+contadorCriacaoBotao+"\");'>"+
						"Alternativa #"+contadorCriacaoBotao+""+
					"</button>"+
				"</h5>"
			);
			var textoCompleto = "";
...
function createQuestionTipo3(questao){
	...
	for(var i=0;i<resposta[questao][0].length;i++){
		if($.isEmptyObject(variacaoQuestao)){
			if(resposta[questao][0].length==1){
				variacaoQuestao.push("<p> char direcaoMovePlayer[15] = \""+resposta[questao][0][i]+"\"; </p>");
				variacaoQuestao.push("<p> moverPlayer (direcaoMovePlayer,"+contadorLocal+"); </p>");
			}else if(resposta[questao][0].length-1==i){
				if(resposta[questao][0][i]==anterior){
					contadorLocal++;
					variacaoQuestao.push("<p> char direcaoMovePlayer[15] = \""+resposta[questao][0][i]+"\"; </p>");
					...
					variacaoQuestao.push("<p> char direcaoMovePlayer[15] = \""+anterior+"\"; </p>");
					variacaoQuestao.push("<p> moverPlayer (direcaoMovePlayer,"+contadorLocal+"); </p>");
					variacaoQuestao.push("<p> direcaoMovePlayer = \""+resposta[questao][0][i]+"\"; </p>");
					variacaoQuestao.push("<p> moverPlayer (direcaoMovePlayer,1); </p>");
				}
			}else if(resposta[questao][0][i]!=anterior){
				if(""==anterior){
					anterior = resposta[questao][0][i];
			...
			if(anterior==resposta[questao][0][i]){
				if(resposta[questao][0].length-1==i){
					contadorLocal++;
					variacaoQuestao.push("<p> direcaoMovePlayer = \""+anterior+"\"; </p>");
					variacaoQuestao.push("<p> moverPlayer (direcaoMovePlayer,"+contadorLocal+"); </p>");
				}else{
					contadorLocal++;
				}
		...
		if($.isEmptyObject(variacaoQuestao)){
			if(resposta[questao][0].length==1){
				...
				if(resposta[questao][0][i]==anterior){
					...
					variacaoQuestao.push("<p> char direcaoMovePlayer[15] = \""+anterior+"\"; </p>");
					variacaoQuestao.push("<p> moverPlayer (direcaoMovePlayer,"+contadorLocal+",\""+resposta[questao][0][i]+"\"); </p>");
					variacaoQuestao.push("<p> moverPlayer (direcaoMovePlayer,1,\"\"); </p>");
				}
			}else if(resposta[questao][0][i]!=anterior){
				if(""==anterior){
...
function createQuestionTipo5(questao){
	...
	for(var i=0;i<resposta[questao][0].length;i++){
		if($.isEmptyObject(variacaoQuestao)){
			if(resposta[questao][0].length==1){
				variacaoQuestao.push("<p> char direcaoMovePlayer[15] = \""+resposta[questao][0][i]+"\"; </p>");
				variacaoQuestao.push("<p> for (char i = 0 ; i < "+contadorLocal+" ; i = i + 1){ </p><p> &nbsp;&nbsp; moverPlayer (direcaoMovePlayer);</p><p> }</p>");
			}else if(resposta[questao][0].length-1==i){
				if(resposta[questao][0][i]==anterior){
					contadorLocal++;
					variacaoQuestao.push("<p> char direcaoMovePlayer[15] = \""+resposta[questao][0][i]+"\"; </p>");
					...
					variacaoQuestao.push("<p> for (char i = 0 ; i < "+contadorLocal+" ; i = i + 1){ </p><p> &nbsp;&nbsp; moverPlayer (direcaoMovePlayer); </p><p> }</p>");
					variacaoQuestao.push("<p> direcaoMovePlayer = \""+resposta[questao][0][i]+"\"; </p>");
					variacaoQuestao.push("<p> for (char i = 0 ; i < 1 ; i = i + 1){ </p><p> &nbsp;&nbsp; moverPlayer (direcaoMovePlayer); </p><p> }</p>");
				}
			}else if(resposta[questao][0][i]!=anterior){
				if(""==anterior){
					anterior = resposta[questao][0][i];
...
function createQuestionTipo6(questao){
	...
	for(var i=0;i<resposta[questao][0].length;i++){
		if($.isEmptyObject(variacaoQuestao)){
			if(resposta[questao][0].length==1){
				variacaoQuestao.push("<p> char direcaoMovePlayer[15] = \""+resposta[questao][0][i]+"\"; </p>");
				variacaoQuestao.push("<p> int contador = "+contadorLocal+";</p>");
				variacaoQuestao.push("<p> while (contador>0){ </p><p> &nbsp;&nbsp; moverPlayer (direcaoMovePlayer); </p><p> &nbsp;&nbsp; contador--; </p><p> }</p>");
			}else if(resposta[questao][0].length-1==i){
				if(resposta[questao][0][i]==anterior){
					contadorLocal++;
	...
	for(var i=0;i<resposta[questao][0].length;i++){
		if($.isEmptyObject(variacaoQuestao)){
			if(resposta[questao][0].length==1){
				variacaoQuestao.push("<p> char direcaoMovePlayer[15] = \""+resposta[questao][0][i]+"\";</p>");
				variacaoQuestao.push("</p> int contador = "+contadorLocal+";</p>");
				variacaoQuestao.push("<p> while (contador>0){ </p><p> &nbsp;&nbsp; moverPlayer (direcaoMovePlayer); </p><p> &nbsp;&nbsp; contador--; </p><p> &nbsp;&nbsp; if (contador==0) { </p><p> &nbsp;&nbsp;&nbsp;&nbsp; direcaoMovePlayer = \"\"; </p><p>&nbsp;&nbsp;}</p><p> }</p>");
			}else if(resposta[questao][0].length-1==i){
				if(resposta[questao][0][i]==anterior){
					contadorLocal++;
					variacaoQuestao.push("<p> char direcaoMovePlayer[15] = \""+resposta[questao][0][i]+"\";</p>");
					...
					variacaoQuestao.push("<p> char direcaoMovePlayer[15] = \""+anterior+"\";</p>");
					variacaoQuestao.push("<p> int contador = "+contadorLocal+";</p>");
					variacaoQuestao.push("<p> while (contador>0){ </p><p> &nbsp;&nbsp; moverPlayer (direcaoMovePlayer); </p><p> &nbsp;&nbsp; contador--; </p><p> &nbsp;&nbsp; if (contador==0) { </p><p> &nbsp;&nbsp;&nbsp;&nbsp; direcaoMovePlayer = \""+resposta[questao][0][i]+"\"; </p><p>&nbsp;&nbsp;}</p><p> }</p>");
					variacaoQuestao.push("<p> contador = 1;</p>");
					variacaoQuestao.push("<p> while (contador>0){ </p><p> &nbsp;&nbsp; moverPlayer (direcaoMovePlayer); </p><p> &nbsp;&nbsp; contador--; </p><p> &nbsp;&nbsp; if (contador==0) { </p><p> &nbsp;&nbsp;&nbsp;&nbsp; direcaoMovePlayer = \"\"; </p><p>&nbsp;&nbsp;}</p><p> }</p>");
				}
			}else if(resposta[questao][0][i]!=anterior){
	...
	for(var i=0;i<resposta[questao][0].length;i++){
		if($.isEmptyObject(variacaoQuestao)){
			if(resposta[questao][0].length==1){
				variacaoQuestao.push("<p> char direcaoMovePlayer[15] = \""+resposta[questao][0][i]+"\";</p>");
				variacaoQuestao.push("<p> int contador = "+contadorLocal+";</p>");
				variacaoQuestao.push("<p> while (contador>0){ </p><p> &nbsp;&nbsp; moverPlayer (direcaoMovePlayer); </p><p> &nbsp;&nbsp; if (contador==0) { </p><p> &nbsp;&nbsp;&nbsp;&nbsp; direcaoMovePlayer = \"\"; </p><p> &nbsp;&nbsp; } else { </p><p> &nbsp;&nbsp;&nbsp;&nbsp; contador--; </p><p> &nbsp;&nbsp; } </p><p> } </p>");
			}else if(resposta[questao][0].length-1==i){
				if(resposta[questao][0][i]==anterior){
					contadorLocal++;
					variacaoQuestao.push("<p> char direcaoMovePlayer[15] = \""+resposta[questao][0][i]+"\";</p>");
					...
					variacaoQuestao.push("<p> char direcaoMovePlayer[15] = \""+anterior+"\";</p>");
					variacaoQuestao.push("<p> int contador = "+contadorLocal+";</p>");
					variacaoQuestao.push("<p> while (contador>0){ </p><p> &nbsp;&nbsp; moverPlayer (direcaoMovePlayer); </p><p> &nbsp;&nbsp; if (contador==0) { </p><p> &nbsp;&nbsp;&nbsp;&nbsp; direcaoMovePlayer = \""+resposta[questao][0][i]+"\"; </p><p> &nbsp;&nbsp; } else { </p><p> &nbsp;&nbsp;&nbsp;&nbsp; contador--; </p><p> &nbsp;&nbsp; } </p><p> } </p>");
					variacaoQuestao.push("<p> contador = 1;</p>");
					variacaoQuestao.push("<p> while (contador>0){ </p><p> &nbsp;&nbsp; moverPlayer (direcaoMovePlayer); </p><p> &nbsp;&nbsp; if (contador==0) { </p><p> &nbsp;&nbsp;&nbsp;&nbsp; direcaoMovePlayer = \"\"; </p><p> &nbsp;&nbsp; } else { </p><p> &nbsp;&nbsp;&nbsp;&nbsp; contador--; </p><p> &nbsp;&nbsp; } </p><p> } </p>");
				}
			}else if(resposta[questao][0][i]!=anterior){
...
function buttonStart(){
	if(controleAcessButton == 0){
		...
		var textoTemporario = "";
		if(resposta[posiRef][1]){
			textoTemporario = "Acertou";
		}else{
			textoTemporario = "Errou";
		}
		funcaoLog(textoTemporario);
	}
}
			...
			if(verificador==1){
				if(limitadorContadorQuestoes>contadorQuestao){
					...
					if(limitadorContadorQuestoes==contadorQuestao){
						...
						"<div class='modal-footer'>Você gostou da fase? <span href='like' class='fa like' style='font-size:25px;'>&#128077;</span> <span href='nolike' class='fa nolike' style='font-size:25px;'>&#128078;</span><br/></div>";
						return 1;
					}else{
						$("#leve"+contadorQuestao).removeClass("level_actual");
						$("#leve"+contadorQuestao).removeClass("level_incorrect");
...
$(document).on('click', '.nolike', function () {
	$(".fa").html("&#128079;");
	$(".fa").removeClass("fa"); 
	funcaoLog('Não gostei');
});

function atualizarContadores(){
	if(contadorQuestao>=3){
		controleQuestaoDif=parseInt(tarefa[0])+1; 
	}else{
		controleQuestaoDif=parseInt(tarefa[0]);
	}
...
function create ()
{
	...
	if(desvio==1&&perfil[1]!=0){
		id_player=registro[0].slice();
		id_obstacle=registro[1];
		id_background=registro[2];
		id_grade=registro[3];
	}else{
		...
		registro[0]=id_player;
		registro[1]=id_obstacle;
		registro[2]=id_background;
		registro[3]=id_grade;
	}
	...
	var posiBackground = Math.floor(Math.random() * (id_background.length));
	...
	if(desvio==0||perfil[1]==0){
		for(var i=0;i<dificuldade*2;i++){
			...
			if(validar==0){
				var posiObstacle = Math.floor(Math.random() * (id_obstacle.length));
		...
		registro[7]=registrarInfoTemp.slice();		
	}else{
		for(var i=0;i<dificuldade*2;i++){
			var posiObstacle = Math.floor(Math.random() * (id_obstacle.length - 1));
			rock.push(this.add.image(registro[7][i][0], registro[7][i][1], object[id_obstacle[posiObstacle]].name).setDisplaySize(47,47));			
		}
	}
